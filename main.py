...
def handle_oauth_callback():
    ...
    # Check for OAuth error
        ...
        # Log OAuth error from callback
        interaction_id = log_oauth_attempt()
        error = query_params.get('error', 'Unknown error')
        error_description = query_params.get('error_description', '')
        
        log_oauth_callback(interaction_id, query_params)
        log_oauth_error(interaction_id, "oauth_callback_error", f"{error}: {error_description}", query_params)
        ...
        if error_values:
            # Log OAuth error from callback (alternative format)
            interaction_id = log_oauth_attempt()
            error_description_values = query_params.get('error_description', [])
            
            log_oauth_callback(interaction_id, query_params)
            log_oauth_error(interaction_id, "oauth_callback_error", f"{error_values[0]}: {error_description_values[0] if error_description_values else ''}", query_params)
    ...
    if AUTHENTICATION_ENABLED and AUTH_AVAILABLE:
        ...
        with st.sidebar:
            ...
            if st.button("🚪 Logout", use_container_width=True):
                ...
                # Log OAuth logout
                interaction_id = log_oauth_attempt()
                log_oauth_success(interaction_id, user_email, user_name)  # Mark as logout success
